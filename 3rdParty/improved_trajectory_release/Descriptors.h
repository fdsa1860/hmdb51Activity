...
void DrawTrack2(const std::vector<Point2f>& point, const std::vector<Point2f>& disp, const int index, const float scale, Mat& image)
{
	Point2f point0 = point[index];
	for (int i = index; i >= 0; --i) {
		point0 = point0 - disp[i];
	}
	point0 *= scale;

	for (int j = 0; j < disp.size(); j++) {
		Point2f point1 = point0 + scale*disp[j];
		line(image, point0, point1, Scalar(0,cvFloor(255.0*(j+1.0)/float(index+1.0)),0), 2, 8, 0);
		point0 = point1;
	}
	circle(image, point0, 2, Scalar(0,0,255), -1, 8, 0);
...
void LoadBoundBox(char* file, std::vector<Frame>& bb_list)
{
    ...
    while(std::getline(bbFile, line)) {
		...
		int size = a.size();
//		std::cout<<frameID<<" "<<size<<std::endl;
